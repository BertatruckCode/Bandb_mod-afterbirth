...
public class ConstructionTreesPalmTree extends WorldGenerator {
	...
	public boolean generate(World world, Random rand, BlockPos position){
		if(    world.getBlockState(new BlockPos(position.getX(), position.getY() +1, position.getZ())).getBlock() == Blocks.air				
			...
			//y = 5 
			 this.changeBlock(world, x + 3 , y + 4, z , leaves);
			 this.changeBlock(world, x - 3 , y + 4, z , leaves);
			 this.changeBlock(world, x , y + 4, z + 3 , leaves);
			 this.changeBlock(world, x , y + 4, z - 3 , leaves);
	
				//y = 6
			 this.changeBlock(world, x + 3 , y + 5, z , leaves);
			 this.changeBlock(world, x - 3 , y + 5, z , leaves);
			 this.changeBlock(world, x , y + 5, z + 3 , leaves);
			 this.changeBlock(world, x , y + 5, z - 3 , leaves);
				
			 this.changeBlock(world, x + 2 , y + 5, z , leaves);
			 this.changeBlock(world, x - 2 , y + 5, z , leaves);
			 this.changeBlock(world, x , y + 5, z + 2 , leaves);
			 this.changeBlock(world, x , y + 5, z - 2 , leaves);
			
			//y = 7
			
			this.changeBlock(world, x+2 , y+6, z ,leaves);
			this.changeBlock(world, x-2 , y+6, z , leaves);
			this.changeBlock(world, x , y+6, z+2 , leaves);
			this.changeBlock(world, x , y+6, z-2 , leaves);
				
			for(int i = 0; i < 3; i++){
				this.changeBlock(world, x-i+1, y+6, z-1  , leaves);
			}
				
			for(int i = 0; i < 3; i++){
				this.changeBlock(world, x-i+1, y+6,z, leaves);
			}
			...
			for(int i = 0; i < 3; i++){
				...
				//y  = 8
			for(int i = 0; i < 3; i++){
				this.changeBlock(world, x-1+i, y+7, z, leaves);
			}
			
			this.changeBlock(world, x, y+7, z+1, leaves);
			this.changeBlock(world, x, y+7, z-1, leaves);
