...
public class GlobalWorldGenerationForest implements IWorldGenerator{
	...
	private void generateSurface(World world, int x, int z, Random rand) {
		// forestHills
		if (world.getBiomeGenForCoords(new BlockPos(x, 16, z)).equals(BiomeGenBase.forest)) {			
			// trees
		...
		// Roofed Forest//
		if (world.getBiomeGenForCoords(new BlockPos(x, 16, z)).equals(BiomeGenBase.roofedForest)) {
			//plants
 			for(int k = 0; k < ListGenerationForestPlant.getListSize(); k++){	
